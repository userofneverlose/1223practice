"""
Модуль для конвертации валют (изолированная бизнес-логика).
Содержит 3 сценария расчёта курса:
1. Прямой курс
2. Обратный курс
...
def _get_usd_rate(db: Session, currency_code: str) -> Optional[Decimal]:
    ...
    Args:
        db: Сессия БД
        currency_code: Код валюты (USD, EUR, RUB...)

    Returns:
        Optional[Decimal]: Курс к USD или None, если нет ни прямого, ни обратного курса

    Пример:
        Курс USD->EUR = 0.92 -> для EUR _get_usd_rate = 0.92
    """
    ...
    if rate_direct:
        return Decimal(str(rate_direct.rate))
...
def convert_currency(
    ...
    Args:
        db: Сессия БД
        from_code: Код исходной валюты (например, "USD")
        to_code: Код целевой валюты (например, "EUR")
    ...
    Raises:
        ...
        ValueError: Если курс не найден (ни по одному сценарию)

    Методы расчёта:
        - "direct": прямой курс из БД
        - "reverse": обратный курс (1 / курс)
        - "cross_usd": кросс-курс через USD
    """
    # 1. Валидация суммы
    if amount <= 0:
    ...
    if rate is None:
        # Получаем объекты курсов из БД
        from_to_usd_obj = get_exchange_rate_by_pair(db, from_code, "USD")
        usd_to_to_obj = get_exchange_rate_by_pair(db, "USD", to_code)

        # Извлекаем Decimal из объектов (или None)
        from_to_usd = from_to_usd_obj.rate if from_to_usd_obj else None
        usd_to_to = usd_to_to_obj.rate if usd_to_to_obj else None

        # Если нет FROM→USD, пробуем обратный: USD→FROM
        if from_to_usd is None:
            usd_to_from_obj = get_exchange_rate_by_pair(db, "USD", from_code)
            if usd_to_from_obj:
                from_to_usd = Decimal("1") / usd_to_from_obj.rate

        # Если нет USD→TO, пробуем обратный: TO→USD
        if usd_to_to is None:
            to_to_usd_obj = get_exchange_rate_by_pair(db, to_code, "USD")
            if to_to_usd_obj:
                usd_to_to = Decimal("1") / to_to_usd_obj.rate

        # Если оба курса найдены — считаем кросс-курс
        if from_to_usd and usd_to_to:
            # Цепочка: FROM → USD → TO (курсы перемножаются)
...
def get_cross_rate_via_usd(
    ...
    """
    Получить кросс-курс через USD (без суммы, просто курс).
    Полезно для демонстрации.

    Args:
        db: Сессия БД
        from_code: Код исходной валюты
        to_code: Код целевой валюты

    Returns:
        Optional[Decimal]: Кросс-курс или None
    """
    try:
