...
Соответствуют ER-диаграмме из docs/er_diagram.md
"""
...
class Currency(Base):
    ...
    # relationship не создает колонку в БД, а позволяет удобно переходить
    # от валюты к курсам: currency.base_rates -> все курсы, где она базовая
    base_rates = relationship(
        "ExchangeRate",
        foreign_keys="[ExchangeRate.base_currency_id]",
        back_populates="base_currency",
        cascade="all, delete-orphan",  # При удалении валюты удаляются и её курсы
    )
