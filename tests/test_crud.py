...
Проверяем работу с базой данных на уровне функций.
"""
...
class TestCurrencyActivation:
    """Тесты для активации/деактивации валют"""
    ...
    def test_deactivate_currency(self, db_session):
        ...
        # 2. Проверяем, что валюта видна обычно
        ...
        # 4. Проверяем, что стала НЕ видна в обычном поиске
        found = crud.get_currency_by_code(db_session, "USD")
        assert found is None

        # 5. Но через include_inactive=True она видна
    ...
    def test_activate_currency(self, db_session):
        """Тест повторной активации валюты"""
        from app.schemas import CurrencyCreate

        # Создаём и сразу деактивируем
        crud.create_currency(
            db_session, CurrencyCreate(code="EUR", full_name="Euro", sign="€")
        )
        crud.deactivate_currency(db_session, "EUR")

        # Проверяем, что не видна
        assert crud.get_currency_by_code(db_session, "EUR") is None

        # Активируем обратно
        result = crud.activate_currency(db_session, "EUR")
        assert result is True

        # Теперь должна быть видна снова
        found = crud.get_currency_by_code(db_session, "EUR")
        assert found is not None
        assert found.is_active == 1

    def test_deactivate_nonexistent_currency(self, db_session):
        """Попытка деактивировать несуществующую валюту"""
        result = crud.deactivate_currency(db_session, "XXX")
