"""
Модульные тесты для калькулятора конвертации валют.
Проверяем все 3 сценария расчёта курса.
"""
...
class TestCalculatorDirectRate:
    ...
    def test_direct_rate_with_high_precision(self, db_session, sample_currencies):
        ...
        assert result == Decimal("92.12")  # Округлилось до 2 знаков
        assert method == "direct"
...
class TestCalculatorReverseRate:
    ...
    def test_reverse_rate_conversion(self, db_session, sample_currencies):
        ...
        Есть курс EUR→USD = 1.086956 (это 1/0.92).
        Конвертируем 100 EUR → должно получиться 108.70 USD.
        """
        from app import crud
        ...
        # Создаём ТОЛЬКО курс USD→EUR (прямой для USD→EUR, но обратный для EUR→USD)
        crud.create_exchange_rate(
            db_session,
            ExchangeRateCreate(
                base_currency_code="USD",
        ...
        # Конвертируем EUR → USD (прямого курса EUR→USD нет, но есть USD→EUR)
        result, method = calculator.convert_currency(
            db_session, "EUR", "USD", Decimal("100")
        )

        # Обратный курс: 1 / 0.92 = 1.086956
...
class TestCalculatorCrossRate:
    ...
    def test_cross_rate_via_usd(self, db_session, sample_currencies):
        ...
        Есть USD→EUR = 0.92 и USD→RUB = 92.50.
        Нет прямого курса EUR→RUB.
        Конвертируем 100 EUR → RUB.
        """
        from app import crud
        ...
        # Конвертируем EUR → RUB (нет прямого курса)
        result, method = calculator.convert_currency(
            db_session, "EUR", "RUB", Decimal("100")
        )

        # Ожидаемый результат: 100 * (92.50 / 0.92) = 10054.3478 → 10054.35
    ...
    def test_cross_rate_missing_usd_link(self, db_session, sample_currencies):
        ...
        # Создаём только курс USD→EUR (нет USD→RUB)
        crud.create_exchange_rate(
            db_session,
            ExchangeRateCreate(
                base_currency_code="USD",
        ...
        # Конвертируем EUR → RUB — невозможно
...
class TestCalculatorEdgeCases:
    ...
    def test_decimal_rounding(self, db_session, sample_currencies):
        ...
        # 100 * 0.9255 = 92.55 (ровно, проверяем что нет ошибок округления)
        result, _ = calculator.convert_currency(
            db_session, "USD", "EUR", Decimal("100")
        )
        assert result == Decimal("92.55")

        # 100 * 0.92555 = 92.555 -> округляется до 92.56
        db_session.query(crud.ExchangeRate).delete()
        crud.create_exchange_rate(
            db_session,
            ExchangeRateCreate(
        ...
        assert result == Decimal("92.56")  # 0.005 округляется вверх
